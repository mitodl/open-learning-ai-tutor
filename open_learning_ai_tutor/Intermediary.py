...
class Intermediary:
    def __init__(
        ...
        self.assessor = (
            Assessor.Assessor(
                self.client, self.model, assessment_history=assessment_history
            )
            if assessor is None
            else assessor
        )
        self.intentSelector = (
            IntentSelector.IntentSelector(intent_history=intent_history)
            if intentSelector is None
            else intentSelector
        )
        self.promptGenerator = (
            PromptGenerator.RigidPromptGenerator()
    ...
    def get_prompt(self, pb, sol, student_messages, tutor_messages, open=True):
        assessment, metadata = self.assessor.assess(
            pb, sol, student_messages, tutor_messages
        )
        ...
        prompt = self.promptGenerator.get_prompt(
            pb, sol, student_messages, tutor_messages, intent, docs
        )
        return (
            prompt,
            intent,
            assessment,
            assessor_prompt_tokens,
            assessor_completion_tokens,
            docs,
            rag_questions,
        )
...
class GraphIntermediary2(Intermediary):
    def __init__(
        ...
        self.intentSelector = (
            IntentSelector.SimpleIntentSelector2(
                intent_history=intent_history, options=options
            )
            if intentSelector is None
            else intentSelector
        )
