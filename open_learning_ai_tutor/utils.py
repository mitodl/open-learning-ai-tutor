...
def json_to_intent_list(json_str):
    ...
    return [[Intent[name] for name in intent_names if name in Intent.__members__] for intent_names in intent_lists]

def intent_list_to_json(intent_lists):
    """Convert a list of Intent enums to a JSON string."""
    intent_names = [[intent.name for intent in intent_list] for intent_list in intent_lists]
...
def json_to_messages(json_messages):
    ...
    for msg in json_messages:
        ...
        additional_kwargs = {k: v for k, v in msg.items() 
