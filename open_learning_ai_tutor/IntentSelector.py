...
class IntentSelector:
    def __init__(self, intent_history=[]) -> None:
        ...
        self.previous_intent = (
            intent_history[-1] if intent_history != [] else [Intent.S_STRATEGY]
        )
    ...
    def get_intent_aux(self, previous_intent, assessment_codes, open_problem=True):
        if (
            not open_problem and "j" in assessment_codes
        ):  # if the problem is not open, then it has only one solution
        ...
        if (
            "j" in assessment_codes
            and "k" not in assessment_codes
            and Intent.G_GREETINGS not in previous_intent
        ):
        ...
        if Intent.S_STRATEGY in previous_intent:
            if (
                "g" in assessment_codes
                and "d" not in assessment_codes
                and "f" not in assessment_codes
            ):
                intents.append(Intent.P_HYPOTHESIS)
            elif (
                "l" in assessment_codes or "k" in assessment_codes
            ) and "g" in assessment_codes:
...
class SimpleIntentSelector2(IntentSelector):
    ...
    def get_intent_aux(self, previous_intent, assessment_codes):
        ...
        if (
            not (Intent.S_CORRECTION in intents)
            and not (Intent.S_SELFCORRECTION in intents)
            and not (Intent.S_OFFLOAD in intents)
            and "j" in assessment_codes
        ):
            intents.append(Intent.G_GREETINGS)
